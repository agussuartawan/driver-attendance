...
@section('content')
    <div class="p-6">
        ...
        <div class="bg-gradient-to-br from-green-600 to-green-500 text-white p-4 rounded-lg mb-6 relative overflow-hidden">
            <div class="flex items-center justify-between relative z-10">
                <div class="text-sm">
                    <div class="font-medium text-xs">Terima kasih untuk hari ini {{ auth()->user()->name }}</div>
        ...
        <div class="space-y-2 text-green-800 font-medium text-xs mb-6 p-4 bg-green-50 rounded-lg">
            ...
            <div class="grid grid-cols-12 gap-2">
                ...
                <div class="col-span-8 flex flex-col items-end justify-end text-right gap-1">
                    ...
                    <span class="text-green-600">Selesai mengantar</span>
        ...
        <div class="space-y-2 text-green-800 font-medium text-xs mb-6 p-4 bg-green-50 rounded-lg">
            <label class="text-green-800 font-semibold text-sm mb-2">Data Pengantaran</label>
            <div class="grid grid-cols-12 gap-2">
                <div class="col-span-4 flex flex-col items-start justify-start gap-1">
                    <span>Nama Tamu :</span>
                    <span>Alamat :</span>
                ...
                <div class="col-span-8 flex flex-col items-end justify-end text-right gap-1">
                    ...
                    <span>{{ $schedule->end_location }}</span>
                </div>
            </div>
        </div>
        ...
        <!-- End Delivery Button -->
        <form action="{{ route('mobile.attendance.create', ['type' => 'out', 'schedule' => $schedule]) }}" method="post" class="mb-6">
            @csrf
            <input type="hidden" name="location" id="location">
            <input type="hidden" name="latitude" id="latitude">
            <input type="hidden" name="longitude" id="longitude">
            <input type="hidden" name="image" id="image">
            <button id="end-delivery-btn" type="submit" disabled class="w-full py-4 px-2 text-md rounded-full border-2 border-green-600 text-green-600 bg-white font-medium text-center transition-all duration-200 hover:bg-green-600 hover:text-white disabled:opacity-50 disabled:cursor-not-allowed">
                Selesai Pengantaran
            </button>
            ...
            <p id="distance-warning" class="mt-2 text-xs text-red-600 hidden">Anda terlalu jauh dari lokasi pengantaran. Jarak maksimal {{ env('ATTENDANCE_MAX_DISTANCE', 10000) }} meter. Silakan mendekati lokasi pengantaran.</p>
    ...
    <script>
        ...
        const scheduleLat = "{{ $schedule->end_latitude }}" === "" ? null : parseFloat("{{ $schedule->end_latitude }}");
        const scheduleLng = "{{ $schedule->end_longitude }}" === "" ? null : parseFloat("{{ $schedule->end_longitude }}");
        ...
        const endBtn = document.getElementById('end-delivery-btn');
        ...
        function updateButtonState() {
            ...
            endBtn.disabled = !(hasLocationPermission && isDistanceValid);
        }
        ...
        const endForm = endBtn.closest('form');
